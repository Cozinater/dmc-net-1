...
class CoviarDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (self._viz == True) and (self._is_train == False):
            ...
            return input_flow, input_mv, input_residual, label, os.path.join(flow_path, flow_tmpl.format('x', idx)), os.path.join(flow_path, flow_tmpl.format('y', idx)), os.path.join(flow_path, img_tmpl.format(idx_first)), classname
        else:
